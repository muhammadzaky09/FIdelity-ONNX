...
def create_random_bitflip_injection(output_name: str, random_value: float):
    """
    Create random bitflip injection using the output tensor's runtime shape.
    This function assumes the output tensor is 3D so that the runtime shape is of length 3.
    It randomly selects one element (a 3D index) and replaces it with the given random_value.
    """
    nodes = []
    ...
    # 2. Generate a random index vector with shape [3] using RandomUniform.
...
def build_test_model():
    ...
    Build a simple test model:
      - Input "x" with shape [1,10,10]
      - Weight: identity matrix of shape [10,10]
      - MatMul producing "y"
      - Then apply random bitflip injection on y.
    """
    # Create input tensor of shape [1,10,10]
    ...
    # Random bitflip injection on y.
    fault_nodes = create_random_bitflip_injection("y", 7.0)  # injecting fault value 7.0; adjust as needed.
    ...
    model = helper.make_model(graph, producer_name="RandomBitflipInjectionTest")
    onnx.checker.check_model(model)
    return model

if __name__ == "__main__":
    model = build_test_model()
    onnx.save(model, "random_bitflip_test.onnx")
