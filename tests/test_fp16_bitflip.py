...
nodes = create_random_bitflip_injection("Y", bit_position=7, fp16=True,
                                        rand_idx_name="rand_idx_inject")

print(f"Node count : {len(nodes)}  (expected 2)")
for n in nodes:
    print(f"  {n.op_type:10s} domain={n.domain!r:20s} inputs={list(n.input)} -> {list(n.output)}")

assert len(nodes) == 2
assert nodes[0].op_type == "Constant"
assert nodes[1].op_type == "BitFlip"
assert nodes[1].domain  == "custom.bitflip"
assert nodes[1].input[0] == "Y"
assert nodes[1].input[1] == "bit_pos_const_rbf"
assert nodes[1].input[2] == "rand_idx_inject"
assert nodes[1].output[0] == "Y_faulty"
